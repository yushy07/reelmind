...
import { probe as probeHardware, queryGpuTelemetry } from './hardware';
import { z } from 'zod';
...
app.whenReady().then(async()=>{
  ...
  handle('gpu-diagnostics', async () => service.renderHardware.diagnosticsReport);
  handle('gpu-telemetry', async () => queryGpuTelemetry(run));
  await service.init();
  if(!selfTest)for(const p of ['gemini','openrouter'] as const)keyPresence[p]=!!await service.key(p).catch(()=>'');
  const hwInfo = await probeHardware(run, service.runtime, service.workers);
