...
export async function run(exe:string,args:string[],options:{signal?:AbortSignal;cwd?:string;input?:string;env?:Record<string,string>;progress?:(line:string)=>void}={}):Promise<string> {
  ...
  return new Promise((resolve,reject)=>{
    ...
    const abort=()=>{if(process.platform==='win32'&&child.pid){const killer=spawn('taskkill.exe',['/PID',String(child.pid),'/T','/F'],{windowsHide:true,stdio:'ignore'});killer.on('error',()=>child.kill());}else child.kill();};options.signal?.addEventListener('abort',abort,{once:true});
    ...
    child.once('error',reject);child.once('close',code=>{options.signal?.removeEventListener('abort',abort);if(options.signal?.aborted)reject(new Error('Processing interrupted'));else if(code!==0)reject(new Error(errors||`Worker exited with code ${code}`));else resolve(output);});
