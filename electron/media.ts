...
export async function probe(runtime:string,file:string,signal?:AbortSignal){
  ...
  return {duration,width:Number(video.width),height:Number(video.height),videoCodec:video.codec_name,audioCodec:audio.codec_name};
}
export async function render(runtime:string,source:string,plan:EditPlan,output:string,work:string,quality:string,signal:AbortSignal,report:(n:number)=>void){
  ...
  const graphs=shots.map((shot,i)=>{
    ...
    return `${trim},scale=${Math.ceil(1080*shot.zoom/2)*2}:${Math.ceil(1920*shot.zoom/2)*2}:force_original_aspect_ratio=increase,crop=1080:1920:x='max(0,min(iw-ow,iw*${shot.center}-ow/2))':y='max(0,(ih-oh)*0.3)',setsar=1,fps=${plan.fps}[o${i}]`;
  });
  ...
  try{await encode(['-c:v','h264_nvenc','-preset','p4','-cq',quality==='high'?'19':'23']);}
  catch{signal.throwIfAborted();await encode(['-c:v','libx264','-preset','fast','-crf',quality==='high'?'19':'23','-threads','4']);}
