...
app.on('browser-window-created',(_event,window)=>{
  ...
  window.webContents.once('did-finish-load',async()=>{
    try {
      ...
      await window.webContents.executeJavaScript("Array.from(document.querySelectorAll('button')).find(b=>b.innerText.toLowerCase().includes('new project')).click()");
      await new Promise(r=>setTimeout(r,500));
      await fs.writeFile('.test-data/import.png',(await window.webContents.capturePage()).toPNG());
      await window.webContents.executeJavaScript("Array.from(document.querySelectorAll('nav button, aside button')).find(b=>b.innerText.toLowerCase().includes('settings')).click()");
      await new Promise(r=>setTimeout(r,200));
      const settingsText=await window.webContents.executeJavaScript('document.body.innerText');
      if(!settingsText.includes('Local Speech Recognition')||!settingsText.includes('Whisper small')||!state.turbo)throw new Error('Transcription settings missing');
      await window.webContents.executeJavaScript("Array.from(document.querySelectorAll('h2')).find(h=>h.innerText.includes('Local Speech Recognition')).scrollIntoView({block:'center',behavior:'instant'})");
