...
$voice.Dispose()
& (Join-Path $PSScriptRoot '../runtime/ffmpeg.exe') -hide_banner -loglevel error -y -f lavfi -i 'color=c=0x272135:s=1280x720:r=30' -i (Join-Path $fixtureRoot 'speech.wav') -shortest -c:v libx264 -preset ultrafast -c:a aac (Join-Path $fixtureRoot 'speech.mp4')
