...
export interface Job { id:string; name?:string; title:string; input:{kind:'local'|'url';value:string;name?:string}; stage:Stage; checkpoint:Stage; progress:number; message:string; createdAt:number; updatedAt:number; cleanupAt?:number; workingDeleted?:boolean; outputs:Reel[]; provider:string; fallbacks?:string[]; error?:string; duration?:number; transcriptionMode?:TranscriptionMode; modelRevision?:string; effectiveTranscriptionMode?:TranscriptionMode; transcriptSource?:'pasted'|'whisper'; transcriptTiming?:TranscriptTimingSource }
...
export interface API { status():Promise<Status>; pickVideo():Promise<string|null>; create(input:CreateInput):Promise<string>; action(id:string,action:'pause'|'resume'|'delete'):Promise<void>; save(id:string):Promise<string|null>; settings(settings:Settings,keys:Partial<Record<Provider,string>>):Promise<void>; setup():Promise<void>; turbo(action:'download'|'cancel'):Promise<void>; subscribe(callback:()=>void):()=>void; onReady(callback:(id:string)=>void):()=>void }
